...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_str):
        nonlocal balance, wrong_pws
        if len(wrong_pws) >= 3:
            return "Too many incorrect attempts. Attempts: " + str(wrong_pws)
...
def merge(incr_a, incr_b):
    ...
    "*** YOUR CODE HERE ***"
    prev = -float("inf")
    while next_a != None or next_b != None: # 不能用not a和not b，因为a可能为0
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    def joint_withdraw(amount, input_str):
        if input_str == old_pass or input_str == new_pass:
            return withdraw(amount, old_pass)
        else:
            return withdraw(amount, input_str)
