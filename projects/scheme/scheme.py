...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args = expr.rest.map(eval_with_env)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        def py_args(args):
            if args == nil:
                return []
            return [args.first] + py_args(args.rest)
        py_args = py_args(args)
        if self.use_env:
            py_args += [env]
        try:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        expr = expressions.rest.first  # expressions: Pair(A, Pair(B, nil)), A:symbol, B:expression to be evaluated
        ...
        lambda_pro = do_lambda_form(Pair(formals, body), env)
        env.define(identifier, lambda_pro)
        return identifier
        # END PROBLEM 9
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    # every element of expressions.rest is a expression of body,
    # so expression.rest is the Scheme list of body expressions.
    # for more details, u could read the code of scheme_read
