...
class Insect:
    ...
    def double_damage(self):
        ...
        self.damage = self.damage * 2  # The damage doubled is a instance attribute, not class attribute
        # to enable this method work properly, the damage attribute should be accessed inside the class by self.damage,
        # instead of <classname>.damage
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        range_to_be = 1  # exclusive upper bound
        while place_to_check.entrance != beehive and (not place_to_check.bees or range_to_be <= self.min_range) and\
                range_to_be <= self.max_range:
            place_to_check = place_to_check.entrance
            range_to_be += 1

        if range_to_be <= self.min_range:
            return None
        return rANTdom_else_none(place_to_check.bees)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor:
            damage += self.damage  # not Fire.damage, cuz we can customize self.damage
            # and do not change state of other objects
...
class HungryAnt(Ant):
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        Ant.__init__(self, armor) # call Parent's constructor
        self.digesting = 0
        self.armor = armor
        # END Problem 6
    ...
    def action(self, gamestate):
        ...
        if self.place.bees:  # if the place has any bee
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        ...
        if not isinstance(other, ContainerAnt):
            return self.contained_ant == None
        return False
        # END Problem 9
    ...
    def double_damage(self):
        if self.contained_ant:
            self.contained_ant.double_damage()
        Ant.double_damage(self)
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, armor = 2):
        ContainerAnt.__init__(self, armor)  # not ContainerAnt(armor), the self is bound to the newly created object
    # END Problem 9
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.is_imposter:
            ...
            cur_place = self.place.exit  # note that damage of queenAnt itself should not be doubled
            while cur_place:
                if cur_place.ant:
                    cur_place.ant.double_damage()
